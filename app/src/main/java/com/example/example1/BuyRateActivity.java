...
public class BuyRateActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_buy_rate);

        Toolbar toolbar = findViewById(R.id.toolbar);
        setSupportActionBar(toolbar);

        SharedPreferences sharedpreferences = getSharedPreferences(MainActivity.MyPREFERENCES, BuyRateActivity.MODE_PRIVATE);
        email=sharedpreferences.getString(Email,"");
        isConnected(BuyRateActivity.this);
        if(!isConnected(BuyRateActivity.this)) buildDialog(BuyRateActivity.this).show();
        else {
            if(email == ""){
                ...
                nDialog = new ProgressDialog(BuyRateActivity.this);
                   ...
                   /* JSONObject NewData =arr.getJSONObject(0);
                    ...
                    JSONArray seg=Table.getJSONArray("Table"); */
                        for (int j = 0; j < arr.length(); j++) {
                            ...
                            String buycash = NewData.getString("buycash");
                            Log.i("Currency name", String.valueOf(j));
                            tname[j].setText(Currency);
                            if(buycash.equals("0")){tcnbuy[j].setText("N/A");}
                            else tcnbuy[j].setText(buycash);
                        }
